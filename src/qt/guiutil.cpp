...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("hydrogen"))
        return false;
...
{
    // Convert hydrogen:// to hydrogen:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hydrogen://"))
    {
        uri.replace(0, 12, "hydrogen:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HydroGen.lnk";
}
...
{
    return GetAutostartDir() / "hydrogen.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HydroGen-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  hydrogen-qt [" + tr("command-line options") + "]                     " + "\n";
